...
if [ "${TRAVIS_PULL_REQUEST}" != "false" ]; then sbt ++$TRAVIS_SCALA_VERSION package; else sbt ++$TRAVIS_SCALA_VERSION rpm:packageBin;curl -T $TRAVIS_BUILD_DIR/druid-manager/target/rpm/RPMS/noarch/druid-manager-1.1.1.1-1.noarch.rpm -u$BINTRAY_USER:$BINTRAY_API_KEY https://api.bintray.com/content/yahoo/rpm/druid-manager/1.1.1.1/druid-manager-1.1.1.1-1.noarch.rpm; fi
#cd $TRAVIS_BUILD_DIR
