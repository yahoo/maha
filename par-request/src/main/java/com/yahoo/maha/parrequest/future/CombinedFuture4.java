...
class CombinedFuture4<T, U, V, W> extends AbstractFuture<Either<GeneralError, Tuple4<T, U, V, W>>> {
        ...
        try {
            ...
            if (returnValue.isLeft()) {
                if (shortCircuit.compareAndSet(false, true)) {
                    set(Either.<GeneralError, T, Tuple4<T, U, V, W>>castLeft(returnValue));
                }
            } else {
    ...
    private void setSecondValue() {
        try {
            ...
            if (returnValue.isLeft()) {
                if (shortCircuit.compareAndSet(false, true)) {
                    set(Either.<GeneralError, U, Tuple4<T, U, V, W>>castLeft(returnValue));
                }
            } else {
    ...
    private void setThirdValue() {
        try {
            ...
            if (returnValue.isLeft()) {
                if (shortCircuit.compareAndSet(false, true)) {
                    set(Either.<GeneralError, V, Tuple4<T, U, V, W>>castLeft(returnValue));
                }
            } else {
    ...
    private void setFourthValue() {
        try {
            ...
            if (returnValue.isLeft()) {
                if (shortCircuit.compareAndSet(false, true)) {
                    set(Either.<GeneralError, W, Tuple4<T, U, V, W>>castLeft(returnValue));
                }
            } else {
