...
class CombinedFuture3<T, U, V> extends AbstractFuture<Either<GeneralError, Tuple3<T, U, V>>> {
        ...
        try {
            ...
            if (returnValue.isLeft()) {
                if (shortCircuit.compareAndSet(false, true)) {
                    set(Either.<GeneralError, T, Tuple3<T, U, V>>castLeft(returnValue));
                }
            } else {
    ...
    private void setSecondValue() {
        try {
            ...
            if (returnValue.isLeft()) {
                if (shortCircuit.compareAndSet(false, true)) {
                    set(Either.<GeneralError, U, Tuple3<T, U, V>>castLeft(returnValue));
                }
            } else {
    ...
    private void setThirdValue() {
        try {
            ...
            if (returnValue.isLeft()) {
                if (shortCircuit.compareAndSet(false, true)) {
                    set(Either.<GeneralError, V, Tuple3<T, U, V>>castLeft(returnValue));
                }
            } else {
