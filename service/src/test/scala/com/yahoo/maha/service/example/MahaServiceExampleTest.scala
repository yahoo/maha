...
class RequestModelSameDimLevelJoinTest extends BaseOracleQueryGeneratorTest {
  ...
  test("Test: Set secondaryDimLevel in the dimension definition; class and batch should not join despite being at the same dimLevel") {
    val jsonString =
      s"""
         |{
         |    "cube": "student_performance",
    ...
    val expected = s"""SELECT  *
                      |      FROM (
                      |          SELECT "Section Name", "Student Name", "Class Name", "Batch Name", ROWNUM AS ROW_NUMBER
                      |              FROM(SELECT s1.name "Section Name", sv3.name "Student Name", c2.name "Class Name", b0.name "Batch Name"
                      |                  FROM
                      |               ( (SELECT  id, student_id, batch_id, class_id, name
                      |            FROM section
                      |            WHERE (student_id = 213)
                      |             ) s1
                      |          INNER JOIN
                      ...
                      |
                      |             ) c2
                      |              ON( s1.class_id = c2.id )
                      |               INNER JOIN
                      |            (SELECT  class_id, name, id
                      |            FROM batch
                      |
                      |             ) b0
                      |              ON( s1.batch_id = b0.id )
                      |               )
                      |
                      |                  ))
