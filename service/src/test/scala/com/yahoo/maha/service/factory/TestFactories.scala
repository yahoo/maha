...
class TestFactEstimator extends FactCostEstimator {
  ...
  override def getSchemaBasedGrainRows(grainKey: String, request: ReportingRequest, filters: scala.collection.mutable.HashMap[String, mutable.Set[Filter]] with scala.collection.mutable.MultiMap[String, Filter], defaultRowCount: Long): Option[Long] = Option(1000)
  override def getAllBasedGrainRows(grainKey: String, request: ReportingRequest, filters: scala.collection.mutable.HashMap[String, mutable.Set[Filter]] with scala.collection.mutable.MultiMap[String, Filter]): Option[Long] = Option(1000)
}

class TestDimEstimator extends DimCostEstimator {
  override def getCardinalityEstimate(grainKey: String, request: ReportingRequest, filters: scala.collection.mutable.HashMap[String, mutable.Set[Filter]] with scala.collection.mutable.MultiMap[String, Filter]): Option[Long] = {
    if(request.isDebugEnabled) {
      Some(10000)
    } else Some(1000)
  }
