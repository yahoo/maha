...
class CombinedFuture3Option<T, U, V>
    ...
    private volatile Option<V> thirdValue = Option.empty();
        ...
        try {
            ...
            if (returnValue.isLeft()) {
                if (allMustSucceed && shortCircuit.compareAndSet(false, true)) {
                    set(EitherUtils.<GeneralError, T, Tuple3<Option<T>, Option<U>, Option<V>>>castLeft(returnValue));
                }
            } else {
    ...
    private void setSecondValue() {
        try {
            ...
            if (returnValue.isLeft()) {
                if (allMustSucceed && shortCircuit.compareAndSet(false, true)) {
                    set(EitherUtils.<GeneralError, U, Tuple3<Option<T>, Option<U>, Option<V>>>castLeft(returnValue));
                }
            } else {
    ...
    private void setThirdValue() {
        try {
            ...
            if (returnValue.isLeft()) {
                if (allMustSucceed && shortCircuit.compareAndSet(false, true)) {
                    set(EitherUtils.<GeneralError, V, Tuple3<Option<T>, Option<U>, Option<V>>>castLeft(returnValue));
                }
            } else {
                thirdValue = Option.apply(returnValue.right().get());
            }
        } catch (CancellationException e) {
            if (allMustSucceed) {
                cancel(false);
