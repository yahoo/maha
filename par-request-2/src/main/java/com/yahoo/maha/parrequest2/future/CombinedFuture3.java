...
/**
 * This class takes three futures and on completion of both, sets a Tuple3<T,U,V> as a result of this future.
 */
class CombinedFuture3<T, U, V> extends AbstractFuture<Either<GeneralError, Tuple3<T, U, V>>> {
    ...
    public CombinedFuture3(ParallelServiceExecutor executor,
        ...
        try {
            ...
            if (returnValue.isLeft()) {
                if (shortCircuit.compareAndSet(false, true)) {
                    set(EitherUtils.<GeneralError, T, Tuple3<T, U, V>>castLeft(returnValue));
                }
            } else {
    ...
    private void setSecondValue() {
        try {
            ...
            if (returnValue.isLeft()) {
                if (shortCircuit.compareAndSet(false, true)) {
                    set(EitherUtils.<GeneralError, U, Tuple3<T, U, V>>castLeft(returnValue));
                }
            } else {
    ...
    private void setThirdValue() {
        try {
            ...
            if (returnValue.isLeft()) {
                if (shortCircuit.compareAndSet(false, true)) {
                    set(EitherUtils.<GeneralError, V, Tuple3<T, U, V>>castLeft(returnValue));
                }
            } else {
    ...
    private void checkAndSet() {
        ...
        if (newRemaining == 0) {
            if (!shortCircuit.get() && !isDone()) {
                ...
                set(new Right<GeneralError, Tuple3<T, U, V>>(new Tuple3<>(firstValue, secondValue, thirdValue)));
            }
        }
    }

    public static <T, U, V> CombinedFuture3<T, U, V> from(ParallelServiceExecutor executor,
        ...
        return new CombinedFuture3<>(executor, firstFuture, secondFuture, thirdFuture);
    }
}
