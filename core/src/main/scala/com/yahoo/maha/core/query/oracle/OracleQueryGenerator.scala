...
class OracleQueryGenerator(partitionColumnRenderer:PartitionColumnRenderer, literalMapper: OracleLiteralMapper = new OracleLiteralMapper) extends BaseQueryGenerator[WithOracleEngine] with OracleQueryCommon with Logging {
...
${queryBuilder.getJoinExpressions}
  ...
  private[this] def generateDimFactOuterGroupByQuery(queryContext: DimFactOuterGroupByQueryQueryContext): Query = {
    val paramBuilder = new QueryParameterBuilder
    val queryBuilderContext = new QueryBuilderContext
    val queryBuilder: QueryBuilder = new QueryBuilder(
      queryContext.requestModel.requestCols.size + 5
