    ...
    ColumnContext.withColumnContext { implicit dc: ColumnContext =>
      Fact.newFact(
        ...
        Set(
          DimCol("account_id", IntType(), annotations = Set(ForeignKey("advertiser")))
          , DimCol("campaign_id", IntType(), annotations = Set(ForeignKey("campaign")))
          , HiveDerDimCol("Day", DateType(), TIME_FORMAT_WITH_TIMEZONE("{utc_hour}", "yyyyMMddHH", "yyyyMMdd"))
          , HiveDerDimCol("Hour", DateType(), TIME_FORMAT_WITH_TIMEZONE("{utc_hour}", "yyyyMMddHH", "yyyyMMddHH"))
          , HiveDerDimCol("derived_day", DateType(), COALESCE("{utc_date}", "'20230101'"))
          , HiveDerDimCol("Derived Day", DateType(), TIME_FORMAT_WITH_TIMEZONE("{derived_day}", "yyyyMMdd", "yyyyMMdd"))
          , HivePartDimCol("frequency", StrType(), partitionLevel = FirstPartitionLevel)
          , HivePartDimCol("utc_date", StrType(), partitionLevel = SecondPartitionLevel)
          , HivePartDimCol("utc_hour", StrType(), partitionLevel = ThirdPartitionLevel)
        ),
