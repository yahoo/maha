...
class PrestoQueryGeneratorV1Test extends BasePrestoQueryGeneratorTest {
  ...
  test("generating presto query") {
    ...
    val expected = s"""SELECT CAST(mang_day as VARCHAR) AS mang_day, CAST(advertiser_id as VARCHAR) AS advertiser_id, CAST(campaign_id as VARCHAR) AS campaign_id, CAST(mang_campaign_name as VARCHAR) AS mang_campaign_name, CAST(ad_group_id as VARCHAR) AS ad_group_id, CAST(keyword_id as VARCHAR) AS keyword_id, CAST(mang_keyword as VARCHAR) AS mang_keyword, CAST(mang_search_term as VARCHAR) AS mang_search_term, CAST(mang_delivered_match_type as VARCHAR) AS mang_delivered_match_type, CAST(mang_impressions as VARCHAR) AS mang_impressions, CAST(mang_ad_group_start_date_full as VARCHAR) AS mang_ad_group_start_date_full, CAST(mang_clicks as VARCHAR) AS mang_clicks, CAST(mang_average_cpc as VARCHAR) AS mang_average_cpc
                      |FROM(
                      |SELECT getFormattedDate(stats_date) mang_day, COALESCE(CAST(account_id as int), 0) advertiser_id, COALESCE(CAST(ssfu0.campaign_id as VARCHAR), 'NA') campaign_id, getCsvEscapedString(CAST(COALESCE(c1.mang_campaign_name, '') AS VARCHAR)) mang_campaign_name, COALESCE(CAST(ad_group_id as int), 0) ad_group_id, COALESCE(CAST(keyword_id as int), 0) keyword_id, getCsvEscapedString(CAST(COALESCE(keyword, '') AS VARCHAR)) mang_keyword, COALESCE(CAST(search_term as VARCHAR), 'None') mang_search_term, COALESCE(CAST(delivered_match_type as varchar), 'NA') mang_delivered_match_type, COALESCE(CAST(impressions as int), 1) mang_impressions, COALESCE(CAST(mang_ad_group_start_date_full as VARCHAR), 'NA') mang_ad_group_start_date_full, COALESCE(CAST(mang_clicks as int), 0) mang_clicks, ROUND(COALESCE((CASE WHEN clicks = 0 THEN 0.0 ELSE CAST(spend AS DOUBLE) / clicks END), 0), 10) mang_average_cpc
  ...
  test("generating presto query with greater than filter") {
    ...
    FROM(
      SELECT COALESCE(CAST(account_id as int), 0) advertiser_id, COALESCE(CAST(impressions as int), 1) mang_impressions
  ...
  test("Verify metric Presto column comparison") {
    ...
    val expected = s"""SELECT CAST(advertiser_id as VARCHAR) AS advertiser_id, CAST(mang_impressions as VARCHAR) AS mang_impressions, CAST(network_id as VARCHAR) AS network_id
                      |FROM(
                      |SELECT COALESCE(CAST(account_id as int), 0) advertiser_id, COALESCE(CAST(impressions as int), 1) mang_impressions, COALESCE(CAST(network_type as VARCHAR), 'NA') network_id
  ...
  test("generating presto query with greater than filter and multiple sort bys") {
    ...
    val expected =
      s"""
         ...
         |FROM(
         |SELECT COALESCE(CAST(account_id as int), 0) advertiser_id, COALESCE(CAST(Count as int), 0) mang_count, COALESCE(CAST(impressions as int), 1) mang_impressions
    ...
    val result =  queryPipelineTry.toOption.get.queryChain.drivingQuery.asInstanceOf[PrestoQuery].asString
        ...
        val expected =
          s"""
             ...
             |FROM(
             |SELECT COALESCE(CAST(ssfu0.account_id as int), 0) advertiser_id, COALESCE(CAST(a1.mang_advertiser_name as VARCHAR), 'NA') mang_advertiser_name, COALESCE(CAST(impressions as int), 1) mang_impressions, ROUND(COALESCE(CASE WHEN ((mang_average_position >= 0.1) AND (mang_average_position <= 500)) THEN mang_average_position ELSE 0.0 END, 0.0), 10) mang_average_position
