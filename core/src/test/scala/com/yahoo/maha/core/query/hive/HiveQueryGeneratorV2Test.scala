...
class HiveQueryGeneratorV2Test extends BaseHiveQueryGeneratorTest {
  ...
  test("generating hive query with outer group by, derived cols") {
    ...
    val expected =
      s"""SELECT CONCAT_WS(',', CAST(NVL(mang_campaign_name,'') AS STRING),CAST(NVL(mang_n_clicks,'') AS STRING),CAST(NVL(mang_pricing_type,'') AS STRING),CAST(NVL(mang_impressions,'') AS STRING),CAST(NVL(mang_n_spend,'') AS STRING),CAST(NVL(mang_month,'') AS STRING),CAST(NVL(mang_clicks,'') AS STRING))
FROM(
...
ORDER BY mang_campaign_name DESC, impressions DESC) OgbQueryAlias
  ...
  test("generating hive query with outer group by, having and partitioning scheme, test part col renderer") {
    ...
    val expected =
      s"""SELECT CONCAT_WS(',', CAST(NVL(mang_campaign_name,'') AS STRING),CAST(NVL(mang_bid_strategy,'') AS STRING),CAST(NVL(mang_advertiser_name,'') AS STRING),CAST(NVL(advertiser_id,'') AS STRING),CAST(NVL(mang_impressions,'') AS STRING),CAST(NVL(mang_test_constant_col,'') AS STRING),CAST(NVL(mang_load_time,'') AS STRING))
FROM(
