...
*/
 ...
 |) OgbQueryAlias
  ...
  test("Query with both aliases with filter and sort by") {
    ...
    val expected =
      s"""
         |SELECT CONCAT_WS(',', CAST(NVL(device_id,'') AS STRING),CAST(NVL(advertiser_id,'') AS STRING),CAST(NVL(mang_ad_format_name,'') AS STRING),CAST(NVL(mang_ad_format_sub_type,'') AS STRING),CAST(NVL(mang_impressions,'') AS STRING),CAST(NVL(mang_pricing_type,'') AS STRING),CAST(NVL(network_id,'') AS STRING))
         |FROM(
 ...
 |       )
  ...
  test("Test specified timezone in additionalParameter should be applied in hive v2 query") {
    val jsonString =
      s"""{
  ...
  test("Test should apply UTC as default timezone in hive v2 query") {
    val jsonString =
      s"""{
                          "cube": "s_stats_timezone",
                          "selectFields": [
