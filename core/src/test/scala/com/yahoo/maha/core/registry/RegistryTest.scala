...
class RegistryTest extends FunSuite with Matchers {
  ...
  test("Get all primitives for a given public fact.") {
    ...
    val domainPrimitives = registry.getFactAndRollups("publicFact", Option.empty, false)
    //assert(domainPrimitives("fact").contains("is_adjustment") && !domainPrimitives("intermediate_rollup").contains("is_adjustment"))

    val allSchemaRollups = registry.getFactAndRollupsWithSchema("publicFact", Option.empty, false, AdvertiserSchema)
    val noopSchemaRollups = registry.getFactAndRollupsWithSchema("publicFact", Option.empty, false, NoopSchema)
    val internalSchemaRollups = registry.getFactAndRollupsWithSchema("publicFact", Option.empty, false, InternalSchema)
    println(domainPrimitives.mkString("\n"))
  }
}
