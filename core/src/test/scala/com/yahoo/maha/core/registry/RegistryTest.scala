...
class RegistryTest extends FunSuite with Matchers {
  ...
  test("Get all columns for a given public fact.") {
    ...
    val expectedAllRollups: Map[String, SortedSet[Column]] = Map("fact" -> allCols.filter(col => col.name != "is_adjustment"), "another_rollup" -> allCols.filter(col => col.name != "stats_source" && col.name != "stats_source"))                //is_adjustment pubCol is Restricted schema.
    val expectedNoopRollups: Map[String, SortedSet[Column]] = Map("intermediate_rollup" -> allCols.filter(col => col.name != "is_adjustment"), "another_rollup" -> allCols.filter(col => col.name != "stats_source" && col.name != "stats_source"))//is_adjustment is discarded on the rollup.
    ...
    val expectedAllButAdjustment: Map[String, SortedSet[Column]] = Map(
      "fact" -> allCols.filter(col => col.name != "is_adjustment"),
      "intermediate_rollup" -> allCols.filter(col => col.name != "is_adjustment")
    )
