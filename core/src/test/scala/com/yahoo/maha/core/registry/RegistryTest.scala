...
class RegistryTest extends AnyFunSuite with Matchers {
  ...
  test("versionedDomainJsonAsString should be able to return the correct json output") {
    ...
    println(versionedJson)
    assert(versionedJson.contains("""name,revision":"advertiser,0"""))
    assert(versionedJson.contains("""name,revision":"advertiser,1"""))
    assert(versionedJson.contains("""name,revision":"publicFact,0"""))
    assert(versionedJson.contains("""name,revision":"publicFact,1"""))
  }
