...
class PrestoQueryGeneratorTest extends BasePrestoQueryGeneratorTest {
  ...
  test("generating presto query") {
    ...
    (
        ",",
        COALESCE(mang_day,''),
        COALESCE(advertiser_id,''),
        COALESCE(campaign_id,''),
        COALESCE(ad_group_id,''),
        COALESCE(keyword_id,''),
        COALESCE(mang_keyword,''),
        COALESCE(mang_search_term,''),
        COALESCE(mang_delivered_match_type,''),
        COALESCE(mang_impressions,'')
    )
FROM
    (
        SELECT
            getformatteddate(stats_date) mang_day,
            CAST(coalesce(account_id,0) AS string) advertiser_id,
            CAST(coalesce(campaign_id,0) AS string) campaign_id,
            CAST(coalesce(ad_group_id,0) AS string) ad_group_id,
            CAST(coalesce(keyword_id,0) AS string) keyword_id,
            getcsvescapedstring(CAST(COALESCE(keyword,'') AS string) ) mang_keyword,
            coalesce(search_term,"None") mang_search_term,
            CAST(coalesce(delivered_match_type,0) AS string) mang_delivered_match_type,
            CAST(coalesce(impressions,0) AS string) mang_impressions
        FROM
            (
                SELECT
                        CASE
                            WHEN (
                                delivered_match_type IN (
                                    1
                                )
                            ) THEN 'Exact'
                            WHEN (
                ...
                WHERE
                    (
                        account_id = 12345
                            ...
                            stats_date >= '2017-10-25'
                        AND
                            stats_date <= '2017-11-01'
                    )
                GROUP BY
                    CASE
                        WHEN (
